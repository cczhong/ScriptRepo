...
while($qindex < scalar(@query_info)) {
  ...
  if($rindex >= $region_info_size || $region_info[$rindex][0] ne $query_info[$qindex][2] || $region_info[$rindex][3] < $query_info[$qindex][4])  {
  ...
  if($qindex < $query_info_size - 1)  {
    while($rindex > 0 && $rindex < $region_info_size && (($region_info[$rindex][0] gt $query_info[$qindex + 1][2]) || ($region_info[$rindex][0] eq $query_info[$qindex + 1][2] && $region_info[$rindex][3] >= $query_info[$qindex + 1][4]))) {
      -- $rindex;
    }
  }
  #print "@{$query_info[$qindex]}\n";
